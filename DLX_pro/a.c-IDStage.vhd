...
entity IDStage is
  ...
  port (
    ...
    -- Forwarding in ID per lo zero_check della branch
    ...
    RS1_OUT         : out std_logic_vector(REG_ADDR-1 downto 0); -- per forwarding/hazard
...
architecture STRUCTURAL of IDStage is
  ...
  -- Forwarding in ID: valore effettivamente confrontato con zero
  signal rega_muxed : std_logic_vector(N-1 downto 0);
...
begin
  ...
  -- MUX di forwarding davanti a zero_check.
  -- Nota: RegA_OUT verso il resto del datapath resta rf_out1 (il forwarding
  -- classico verso EX gestisce le istruzioni ALU/load/store). Qui il MUX
  -- serve solo alla branch resolution.
  rega_muxed <= EXMEM_ALU_OUT_IN when FWD_ID_A = "10" else
                WB_DATA_IN       when FWD_ID_A = "01" else
                rf_out1;

  U_ZERO_CHECK: zero_check
    generic map (NBIT => N)
    port map (
      D_IN     => rega_muxed, -- (RegA con forwarding se necessario)
      OPCODE   => opcode,
