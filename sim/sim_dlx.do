# ============================================================================
#  sim_dlx.do  --  ModelSim / QuestaSim automation script for the DLX-pro
#
#  Usage, from the ModelSim transcript:
#      cd <repo>/sim
#      do sim_dlx.do
#
#  Re-run the same command after editing any .vhd; everything is recompiled.
#  The IRAM opens "test.asm.mem" relative to the current directory, so this
#  script must be run from sim/.
# ============================================================================
...
if { ![file exists test.asm.mem] } {
    echo "ERROR: test.asm.mem not found in [pwd]."
    echo "       Run this script from the sim/ directory."
    return
}
...
# ---- 2. Compilation (hierarchical, bottom-up) ------------------------------
echo "======== Package ========"
vcom -quiet $SRC/000-globals.vhd

echo "======== Generic primitives (src/common) ========"
vcom -quiet $SRC/common/01-reg_en.vhd
vcom -quiet $SRC/common/01-G_block.vhd
vcom -quiet $SRC/common/01-PG_block.vhd
vcom -quiet $SRC/common/01-pg_network.vhd
vcom -quiet $SRC/common/01-koggle_stone_adder.vhd
vcom -quiet $SRC/common/01-shifter.vhd
vcom -quiet $SRC/common/01-logic.vhd
vcom -quiet $SRC/common/01-comparator_eq.vhd
vcom -quiet $SRC/common/01-zero_check.vhd
vcom -quiet $SRC/common/01-ADD_SUB.vhd

echo "======== IF-stage sub-modules ========"
vcom -quiet $SRC/core/a.b-IFStage.core/a.b.a-PC_sel.vhd
vcom -quiet $SRC/core/a.b-IFStage.core/a.b.b-BTB.vhd
vcom -quiet $SRC/core/a.b-IFStage.core/a.b.c-Icache.vhd
vcom -quiet $SRC/core/a.b-IFStage.core/a.b.d-IRAM.vhd

echo "======== ID-stage sub-modules ========"
vcom -quiet $SRC/core/a.c-IDStage.core/a.c.a-RF.vhd
vcom -quiet $SRC/core/a.c-IDStage.core/a.c.b-ImmGen.vhd
vcom -quiet $SRC/core/a.c-IDStage.core/a.c.c-branch_resolution.vhd

echo "======== EX-stage sub-modules ========"
vcom -quiet $SRC/core/a.d-EXStage.core/a.d.a-ALU.vhd

echo "======== MEM-stage sub-modules ========"
vcom -quiet $SRC/core/a.e-MEMStage.core/a.e.a-DRAM.vhd

echo "======== Pipeline registers ========"
vcom -quiet $SRC/core/a.g-IF_ID_reg.vhd
vcom -quiet $SRC/core/a.h-ID_EX_reg.vhd
vcom -quiet $SRC/core/a.i-EX_MEM_reg.vhd
vcom -quiet $SRC/core/a.j-MEM_WB_reg.vhd

echo "======== Pipeline stages ========"
vcom -quiet $SRC/core/a.b-IFStage.vhd
vcom -quiet $SRC/core/a.c-IDStage.vhd
vcom -quiet $SRC/core/a.d-EXStage.vhd
vcom -quiet $SRC/core/a.e-MEMStage.vhd
vcom -quiet $SRC/core/a.f-WBStage.vhd

echo "======== Forwarding + hazard detection ========"
vcom -quiet $SRC/core/a.k-forwarding_unit.vhd
vcom -quiet $SRC/core/a.l-forwarding_id.vhd
vcom -quiet $SRC/core/a.m-hazard_detector.vhd

echo "======== Control Unit ========"
vcom -quiet $SRC/core/a.a-CU.vhd

echo "======== Top level + testbench ========"
vcom -quiet $SRC/core/a-DLX.vhd
vcom -quiet tb/TB_DLX.vhd
...
echo "  Program: sw/asm/test_pro_stress.asm (45 instructions)."
