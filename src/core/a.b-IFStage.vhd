...
entity IFStage is
  ...
  port (
    ...
    TAKEN_OUT           : out std_logic; -- predizione BTB latchata verso ID
...
architecture STRUCTURAL of IFStage is
  ...
  component pc_mux
    ...
    port (
        ...
        branch_npc_in       : in  std_logic_vector(N-1 downto 0);
        mispredict_in       : in  std_logic;
        actual_taken_in     : in  std_logic;
  ...
  -- NPC del branch che si trova in ID (= PC del branch + 4).
  -- Serve come indirizzo di recupero quando il branch era predetto taken ma NON salta.
  signal branch_npc   : std_logic_vector(PC_SIZE-1 downto 0);
...
begin
  ...
  -- NPC del branch in ID: pc_update_in e' il PC del branch in Decode
  ...
  U_PC_MUX: pc_mux
    ...
    port map (
      ...
      branch_npc_in       => branch_npc,
      mispredict_in       => mispredict_in,
      actual_taken_in     => actual_taken_in,
      taken_in            => btb_taken,
      next_pc_out         => PC_next
    );
