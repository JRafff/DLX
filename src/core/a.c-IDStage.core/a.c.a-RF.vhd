...
begin
  ...
  -- Lettura asincrona con bypass di scrittura (write-through)

  OUT1 <= (others => '0') when unsigned(ADD_RD1) = 0 else
          DATAIN          when (WR = '1' and ADD_WR = ADD_RD1) else
          REGISTERS(to_integer(unsigned(ADD_RD1)));
  OUT2 <= (others => '0') when unsigned(ADD_RD2) = 0 else
          DATAIN          when (WR = '1' and ADD_WR = ADD_RD2) else
          REGISTERS(to_integer(unsigned(ADD_RD2)));
